...
const recentCommentsResponse: PaginatedResponse<AdminCommentItem> = {
  data: [
    {
      id: 1,
      postId: 101,
...
function DashboardStoryProvider({
  children,
}: {
  ...
  const [queryClient] = useState(() => {
    ...
    client.setQueryData(["dashboard", "stats"], dashboardStats);
    client.setQueryData(["dashboard", "recentComments"], recentCommentsResponse);
...
function DashboardCanvas({ withShell }: { withShell: boolean }) {
  ...
  return <DashboardStoryProvider>{content}</DashboardStoryProvider>;
}
