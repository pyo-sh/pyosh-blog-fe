  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap gap-3">
        ...
        <label className="flex min-w-[8rem] flex-col gap-1.5 text-xs text-text-3">
          <span className="font-medium text-text-2">작성자</span>
          <select
            value={authorType}
            onChange={(event) =>
              onAuthorTypeChange(
        ...
        <label className="flex min-w-[10rem] flex-col gap-1.5 text-xs text-text-3">
          <span className="font-medium text-text-2">시작일</span>
          <input
            type="date"
            value={startDate}
        ...
        <label className="flex min-w-[10rem] flex-col gap-1.5 text-xs text-text-3">
          <span className="font-medium text-text-2">종료일</span>
          <input
            type="date"
            value={endDate}
          ...
          <form onSubmit={handleSubmit} className="flex flex-wrap gap-2">
            <div className="relative">
              <input
                ...
                className="w-[16rem] rounded-[0.75rem] border border-border-3 bg-background-1 px-3 py-2 pr-9 text-sm text-text-1 outline-none transition-colors focus:border-primary-1 placeholder:text-text-4"
              />
              {searchInput ? (
                <button
                  type="button"
      ...
      {searchQuery ? (
        <div className="flex flex-wrap items-center gap-2 text-sm text-text-3">
          <span className="rounded-full bg-background-1 px-3 py-1.5 text-text-2">
            &apos;{searchQuery}&apos; 검색 결과
          </span>
          <button
            type="button"
